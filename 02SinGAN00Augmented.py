exp_name="02SinGAN00Augmented" # from the series of SinGAN augmented, where the dataset is actually augmented.
batch_size=16
lr=1e-5
ita_threshold=41
seed = 2307
...
print("There are a total of {} samples in the train set".format(len(df_train)))
# add synthetic images before splittting
df_synth = prepare_df(TRAINING_PATH,"synthetic_images.csv")
df_train = pd.concat((df_train,df_synth),ignore_index=True)
print("The dataset has been augmented")
print("There are a total of {} samples in the train set".format(len(df_train)))
